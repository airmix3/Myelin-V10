...
- **CSS**: Tailwind 4 with V2 glass-morphism design system in globals.css. Glass utility classes (.glass-panel, .glass-card, .glass-deep, .glass-pill). No Bootstrap or other CSS frameworks.
...
|-------|-----|-------------|
| Bootstrap / any CSS framework (except Tailwind) | Tailwind 4 is the only CSS framework. No Bootstrap, Foundation, etc. | Tailwind 4 + V2 glass utility classes. |
| framer-motion | Deprecated package name. V2 uses the rebranded `motion` package. | `motion` (import from `motion/react`) |
