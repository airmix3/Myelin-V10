...
export function truncateText(text: string, maxLen = 180): string {
...
export function extractToolUseBlocks(message: SDKAssistantMessage): ToolUseBlock[] {
  const content = message.message?.content;
  if (!Array.isArray(content)) return [];
...
export async function invokeAgent(opts: InvokeAgentOptions): Promise<InvokeAgentResult> {
  ...
  // Tamir-specific config: skill-creator plugin + PreCompact hook
  if (opts.agentId === 'tamir') {
    ...
    // D-16: PreCompact hook -- nudge to save context before SDK compaction
    queryOptions!.hooks = {
      PreCompact: [{
        hooks: [async (_input: unknown, _toolUseId: string | undefined, _options: { signal: AbortSignal }) => ({
          continue: true,
          systemMessage: [
            '[System: Context is being compacted to free up space.',
            'Before compaction completes, save anything worth remembering to your persistent memory using the memory tool.',
            'Focus on: CEO preferences learned, company conventions discovered, routing patterns, active priorities, lessons learned.',
            'Your frozen memory snapshot in the system prompt will survive compaction automatically.]',
          ].join(' '),
        })],
      }],
    };
      ...
      for await (const msg of q) {
        ...
        if (msg.type === 'result') {
          if (msg.subtype === 'success') {
            ...
            try {
              // Read subtaskId from the run for cost attribution
              const runForCost = sqlite.prepare('SELECT subtask_id FROM task_runs WHERE id = ?').get(opts.runId) as { subtask_id: string | null } | undefined;
              const subtaskIdForCost = runForCost?.subtask_id ?? null;

              sqlite.prepare(`
                INSERT INTO cost_events (id, taskId, agentId, inputTokens, outputTokens, costUsd, model, run_id, subtask_id, createdAt)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'))
              `).run(
                generateId('cost'),
                opts.taskId,
...
/**
 * Streaming variant of invokeAgent for warm session support.
 * Accepts an AsyncIterable<SDKUserMessage> as prompt and returns the Query object
 * directly for the caller (WarmSessionManager) to iterate.
 * Does NOT iterate the generator -- the caller manages that.
 */
export interface InvokeAgentStreamingOptions extends Omit<InvokeAgentOptions, 'prompt'> {
  prompt: AsyncIterable<SDKUserMessage>;
}

export interface InvokeAgentStreamingResult {
  query: ReturnType<typeof query>;
  sessionIdPromise: Promise<string>;
}

export function invokeAgentStreaming(opts: InvokeAgentStreamingOptions): InvokeAgentStreamingResult {
  const log = logger.child({ module: 'invoke-agent-streaming', taskId: opts.taskId, agentId: opts.agentId });
  ...
  log.info({ deskDir: opts.deskDir, sessionId: opts.sessionId ?? 'new' }, 'Starting streaming agent invocation');

  // 4. Build query options (same as invokeAgent)
  const queryOptions: Parameters<typeof query>[0]['options'] = {
    pathToClaudeCodeExecutable: process.env.CLAUDE_CODE_PATH ?? '/home/omersh/.npm-global/bin/claude',
    systemPrompt: { type: 'preset', preset: 'claude_code', append: opts.soulMd },
    cwd: opts.deskDir,
  ...
  // Restrict built-in tools when specified
  if (opts.tools !== undefined) {
    queryOptions!.tools = opts.tools;
  }
  ...
  // 5. Create query with AsyncIterable prompt
  const q = query({ prompt: opts.prompt, options: queryOptions });

  // Register query ref for hot-reload access
  registerQueryRef(opts.runId, q);

  // Create a deferred promise for sessionId -- resolved when init message is seen by consumer
  let resolveSessionId: (id: string) => void;
  const sessionIdPromise = new Promise<string>((resolve) => {
    resolveSessionId = resolve;
  });

  // Expose the sessionId resolver on the query object for the consumer loop
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  (q as any).__resolveSessionId = resolveSessionId!;
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  (q as any).__runId = opts.runId;

  return { query: q, sessionIdPromise };
}
