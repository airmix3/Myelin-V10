...
export async function POST(
  ...
  try {
    ...
    await invokeAgent({
      ...
      agentId: 'tamir',
      department: 'cos',
      prompt: `Follow your skill-tool-manager skill. Refresh skill: ${skill.name} at path ${skill.filePath}. Department: ${dept}.`,
    ...
    return NextResponse.json({ success: true, message: `Skill ${skill.name} refresh started` });
  } catch (err) {
