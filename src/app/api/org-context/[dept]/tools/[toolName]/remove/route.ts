...
export async function POST(
  request: NextRequest,
  { params }: { params: { dept: string; toolName: string } }
) {
  const { dept, toolName } = params;
  ...
  try {
    ...
    await invokeAgent({
      ...
      agentId: 'tamir',
      department: 'cos',
      prompt: `Follow your skill-tool-manager skill. Remove tool: ${toolName} from directory data/departments/${dept}/tools/${toolName}/. Stop any running MCP server processes for this tool. Department: ${dept}.`,
    ...
    return NextResponse.json({ success: true, message: `Tool ${toolName} removed` });
  } catch (err) {
