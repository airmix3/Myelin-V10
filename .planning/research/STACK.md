...
**Domain:** TypeScript AI Agent Operating System (self-hosted Next.js + Claude Agent SDK)
...
```bash
# Core framework
pnpm add next@14.2.35 react react-dom

# AI + Agent
pnpm add @anthropic-ai/claude-agent-sdk@0.2.83 @anthropic-ai/sdk@0.80.0

# Database
pnpm add prisma@7.5.0 @prisma/client@7.5.0 @prisma/adapter-better-sqlite3@7.5.0 better-sqlite3@12.8.0

# Protocol + Validation
pnpm add @a2a-js/sdk@0.3.13 @modelcontextprotocol/sdk@1.28.0 zod@4.3.6

# Content processing
pnpm add gray-matter@4.0.3 marked@17.0.5

# Infrastructure
pnpm add pino@10.3.1 proper-lockfile@4.1.2 node-cron@4.2.1

# Dev dependencies
pnpm add -D typescript@6.0.2 vitest@4.1.1 pino-pretty@13.1.3 @types/better-sqlite3@7.6.13 @types/node@22 @types/react @types/react-dom
```
...
**For FTS5 operations (search, vault, knowledge):**
...
**For SSE streaming (build log, task transitions, heartbeat):**
...
**For agent execution (all agent invocations):**
...
**For structured output (routing, turn types):**
...
Zod v4 is a ground-up rewrite. Key differences:
...
You will use TWO database access patterns:
1. **Prisma Client** for all standard CRUD (employees, tasks, deliverables, etc.)
2. **better-sqlite3 direct** for FTS5 tables, triggers, and raw performance-critical queries

Both access the same `.db` file. Use WAL mode (`PRAGMA journal_mode=WAL`) set once at startup via better-sqlite3 for safe concurrent reads.

### Claude Agent SDK is NOT claude-code-sdk
The package is `@anthropic-ai/claude-agent-sdk` (v0.2.83), NOT `@anthropic-ai/claude-code-sdk` (which does not exist on npm). The Agent SDK builds on Claude Code's capabilities but is a separate package for programmatic agent building.
...
---
*Stack research for: Myelin v10 -- TypeScript AI Agent OS*
*Researched: 2026-03-25*
